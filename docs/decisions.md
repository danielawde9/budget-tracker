...
## 2026-09-20 — Schedule/goal references are picked, and income amounts can link to the monthly plan

**Decision:** The schedule editor's category/loan/funding-goal/preferred-wallet references and the confirm-payment dialog's paying wallet are native `<select>` dropdowns fed by injected reference-option lists (`ScheduleReferenceOptions` in `src/features/recurring/schedule-editor.tsx`, assembled in `src/features/control-room/routes.tsx`), replacing the pasted-UUID text fields. The funding-goal dropdown lists active goals from both currencies, currency-suffixed, because a schedule's currency choice is independent of the goal it funds. Separately, income schedules (amount source) and goals on `manual_monthly` contributions can bind their amount to the monthly plan's `plannedIncomeMinor` for their currency — a snapshot of the current planned-income revision saved as the schedule's `expectedMinor`/goal's `monthlyAmountMinor`, with the custom-amount path unchanged. This supersedes the "references are pasted ids, never a picker" scope boundary recorded below (2026-09-14 task entry, item 2 at former line ~2664): the picker data is now plumbed through `routes.tsx`, so that reason no longer applies. The confirm-payment "Transaction reference id" (link-existing mode) remains a pasted id with a described hint, because no candidate-event list is available to that dialog — only the occurrence row is in scope there.

**Why:** Pasting UUIDs is an error-prone developer-facing pattern, not an operator-facing one; dropdowns validate by construction and stay server-validated regardless. Linking amounts to the planned income makes the plan the single source of truth for "what's coming in" and removes duplicate manual entry, while snapshotting at save keeps the existing immutable-revision command shapes (`save_schedule`, goal definitions) unchanged — no schema or RPC changes were needed.

**If changed:** Dynamic (re-evaluating) amount linkage instead of snapshot would require new command semantics or schema (source columns on schedule/goal revisions) plus projection changes. Restoring free-text id entry would reintroduce client UUID validation and drop the picker's by-construction correctness. A picker for the confirm-payment transaction reference needs a new bounded read (candidate linkable events per wallet) with its own authorization and pagination design.

## 2026-09-20 — Manage/settings presentation overhaul on the Control Room design system

**Decision:** The Manage destination (hub, Wallets, Categories, Loans, Household and their dialogs) was restyled onto the existing `--cr-*` token system: balance-first wallets workspace with a styled archived-disclosure, per-currency loans summary cards with localized history labels (fixing the raw `kind.replaceAll` display), card-based category registers, and an icon-led settings hub with grouped Preferences/Account sections. Each area ships a feature-scoped stylesheet (`wallets-workspace.css`, `loans-workspace.css`, `categories-manage.css`, `manage-hub.css`) using only design tokens, logical properties, and `prefers-reduced-motion`; the legacy global sheets were left in place. All accessible names, dialog titles, focus-return, 390px containment, and RTL contracts were held stable — verified by the existing unit and e2e suites.

**Why:** The Manage pages were legacy full-desktop pages papered into the shell via `.cr-main` normalization rules, with three divergent modal implementations, four i18n patterns, and inconsistent empty states. Rebuilding the presentation on tokens with per-feature sheets removes the specificity fights without touching protected commands, gateways, balance rules, or archive behavior (presentation and route composition only).

**If changed:** A future consolidation may merge the four feature sheets into one settings sheet and delete the now-orphaned legacy rules in `styles.css`; unifying the loans/household modal frames onto the shared wallets `DialogShell` requires preserving the nested-dialog `active` retention and `[data-initial-focus]` behavior that e2e focus-return regressions pin. Category restore remains impossible in the UI until a `restore_category` RPC exists (only `archive_category` does); listing archived categories needs the gateway/hook to stop hard-filtering `archived_at`.
