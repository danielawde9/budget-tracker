...
describe('AuthScreen', () => {
  it('submits visibly labelled sign-in fields and preserves email on failure', async () => {
    ...
    render(<AuthScreen locale="en" state="signed-out" error="Sign-in was not accepted." pending={false} onLocaleChange={vi.fn()} onSignIn={onSignIn} onSignUp={vi.fn()} />);
  ...
  it('switches to sign-up and announces email confirmation without credentials', async () => {
    const user = userEvent.setup();
    const { rerender } = render(<AuthScreen locale="en" state="signed-out" error={null} pending={false} onLocaleChange={vi.fn()} onSignIn={vi.fn()} onSignUp={vi.fn()} />);
    await user.click(screen.getByRole('button', { name: 'Create account' }));
    expect(screen.getByRole('heading', { name: 'Create your account' })).toBeInTheDocument();
    expect(screen.getByLabelText('Password')).toHaveAttribute('autocomplete', 'new-password');

    rerender(<AuthScreen locale="en" state="confirmation-required" confirmationEmail="new@example.com" error={null} pending={false} onLocaleChange={vi.fn()} onSignIn={vi.fn()} onSignUp={vi.fn()} />);
  ...
  it('renders equivalent Arabic RTL controls', () => {
    render(<AuthScreen locale="ar" state="expired" error={null} pending={false} onLocaleChange={vi.fn()} onSignIn={vi.fn()} onSignUp={vi.fn()} />);
