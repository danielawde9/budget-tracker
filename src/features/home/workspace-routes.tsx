...
export function WorkspaceRoutes(props: WorkspaceRoutesProps) {
  ...
  if (props.activeDestination === 'wallets') return <Suspense fallback={<div className="state-panel" role="status">{props.locale === 'ar' ? 'جارٍ تحميل المحافظ…' : 'Loading Wallets…'}</div>}><WalletsPage categoriesGateway={props.categoriesGateway} locale={props.locale} spaceId={props.spaceId} userId={props.userId} onSpaceUnavailable={props.onSpaceUnavailable} walletState={walletState} openTransaction={props.openTransaction} onTransactionDialogOpened={props.onTransactionDialogOpened} onOpenLoans={() => props.onDestinationChange('loans')} /></Suspense>;
