...
function LoanRow({ loan, locale, onOpen }: { loan: Loan; locale: Locale; onOpen: (loan: Loan) => void }) {
  return (
    <li className="loan-row">
      <button type="button" className="loan-row-button" onClick={() => onOpen(loan)} aria-label={locale === 'ar' ? `فتح قرض ${loan.personName}` : `Open ${loan.personName} loan`}>
