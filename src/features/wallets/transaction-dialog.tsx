  ...
  return <DialogShell title={t(props.locale, 'Add a transaction', 'إضافة معاملة')} closeLabel={t(props.locale, 'Close', 'إغلاق')} onClose={props.onClose} pending={props.pending || refreshing} wide>
    <form className="transaction-form" onSubmit={review}>
      ...
      <div className="form-grid">
        ...
        <label className="full-field">{t(props.locale, 'Amount', 'المبلغ')}<input inputMode="decimal" placeholder="0.00" value={amount} disabled={mutationLocked} onChange={(event) => edit(() => setAmount(event.target.value))} /></label>
        <label>{t(props.locale, 'Payee', 'الجهة')}<input placeholder={t(props.locale, 'e.g. Market', 'مثال: متجر')} value={payeeName} list="transaction-payees" maxLength={120} disabled={mutationLocked} onChange={(event) => edit(() => { const nextPayee = event.target.value; setPayeeName(nextPayee); if (kind === 'income' || kind === 'expense') setCategoryId(suggestedCategoryForPayee(props.recentEvents ?? [], nextPayee, kind, eligibleCategories)); })} />{(props.payees?.length ?? 0) > 0 && <datalist id="transaction-payees">{props.payees?.map((payee) => <option key={payee.id} value={payee.name} />)}</datalist>}{payeeSuggestion && <span className="field-note" role="status">{t(props.locale, 'Category suggested from 2 of your last 3 entries. You can change it.', 'اقتُرحت الفئة من اثنين من آخر ثلاثة قيود. يمكنك تغييرها.')}</span>}</label>
        <label className="full-field">{t(props.locale, 'Note', 'ملاحظة')}<textarea placeholder={t(props.locale, 'Optional', 'اختياري')} value={note} maxLength={2000} rows={3} disabled={mutationLocked} onChange={(event) => edit(() => setNote(event.target.value))} /></label>
      </div>
