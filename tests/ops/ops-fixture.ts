...
export function makeBackupFixture() {
  ...
  makeExecutable(
    ...
    [
      ...
      'fi',
      'if [[ "$counter" == "1" && -n "${BUDGET_FAKE_SWAP_DESCENDANT_PATH:-}" ]]; then',
      '  mv -- "$BUDGET_FAKE_SWAP_DESCENDANT_PATH" "${BUDGET_FAKE_SWAP_DESCENDANT_PATH}.validated"',
      '  ln -s -- "$BUDGET_FAKE_SWAP_DESCENDANT_TARGET" "$BUDGET_FAKE_SWAP_DESCENDANT_PATH"',
      'fi',
      'if [[ "$counter" == "1" && -n "${BUDGET_FAKE_SWAP_EXECUTABLE_PATH:-}" ]]; then',
      '  mv -- "$BUDGET_FAKE_SWAP_EXECUTABLE_PATH" "${BUDGET_FAKE_SWAP_EXECUTABLE_PATH}.validated"',
  ...
  const env: NodeJS.ProcessEnv = {
    ...
    BUDGET_TIMEOUT_SHA256: fileSha256(timeout),
    BUDGET_AGE_SHA256: fileSha256(age),
    BUDGET_CATALOG_SHA256: fileSha256(catalog),
    BUDGET_OFFSITE_SHA256: fileSha256(offsite),
    BUDGET_CLOCK_SHA256: fileSha256(clock),
...
export function makeRestoreFixture() {
  ...
  const env: NodeJS.ProcessEnv = {
    ...
    BUDGET_OFFSITE_SHA256: fileSha256(backup.env.BUDGET_OFFSITE_BIN as string),
    BUDGET_ROLE_FILTER_BIN: roleFilter,
    BUDGET_ROLE_FILTER_SHA256: fileSha256(roleFilter),
    BUDGET_COMPARE_BIN: compare,
    BUDGET_COMPARE_SHA256: fileSha256(compare),
